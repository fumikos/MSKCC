...
dim(glioma_HSP90_NQ_high)
res<-glioma_HSP90_NQ_high[order(glioma_HSP90_NQ_high[,3])[1:5],]
sen<-glioma_HSP90_NQ_high[order(glioma_HSP90_NQ_high[,3])[21:25],]
...
head(sen_sig)
write.table(sen_sig$SYMBOL, file="glioma_sen_sig_NQ.txt", quote=F, row.names=F, col.names=F)

res_sig<-gene_name[fit$coefficients[,2]<0,]
dim(res_sig)
res_sig<-res_sig[order(fit$p.value[fit$coefficients[,2]<0, 2])[1:100],]
head(res_sig)
write.table(res_sig$SYMBOL, file="glioma_res_sig_NQ.txt", quote=F, row.names=F, col.names=F)
...
write.table(multi_sig$SYMBOL, file="glioma_multitest_NQ.txt", quote=F, row.names=F, col.names=F)
