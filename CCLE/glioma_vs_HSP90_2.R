...
elasticnet<-lapply(alphaslist, function(a){
  cv.glmnet(X, Y, alpha=a, nfolds=8, foldid=fold_id,
            lambda=grid,parallel = TRUE)})
mypar(3,4)
...
for(i in 1:200){
     idx <- sample(length(Y), replace = T)
     for(j in 1:length(Y)){
          X_resample2[j,] <- X[idx[j],]
          Y_resample2[j] <- Y[idx[j]]}
...
for(i in 1:200){
     idx <- sample(length(Y), replace = T)
     for(j in 1:length(Y)){
          X_resample3[j,] <- X[idx[j],]
          Y_resample3[j] <- Y[idx[j]]}
...
CV_B <- matrix(data=NA, nrow=length(Y), ncol=2)
for(j in 1:2){
...
# Expression level of Rhodopsin
mean(rearranged["6010_at",])

# Predicting response of U251 and LN229

head(glioma_Exp)
dim(glioma_Exp)
class(glioma_Exp)

X_all_gliomas <- matrix(glioma_Exp, nrow = 18988, ncol = 69) 
# rearranged = glioma_Exp[,idx] = X_all_gliomas[,idx] = X
X_all_gliomas<-t(X_all_gliomas)%>%scale
colMeans(X_all_gliomas)
rowSds(t(X_all_gliomas))
dim(X_all_gliomas)
X_all_gliomas<-X_all_gliomas[,idx_by_R]
X_all_gliomas[idx,1:5]
X[1:5,1:5]
dim(X)
dim(X_all_gliomas[idx,])

fit <- glmnet(X_all_gliomas[idx,], Y, alpha = a, lambda=grid)
test_pred<-predict(fit, X_all_gliomas[-idx,], type = "response", s = mol)
rownames(test_pred)<-colnames(glioma_Exp)[-idx]
test_pred
