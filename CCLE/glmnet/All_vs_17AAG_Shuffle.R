...
Y_sh <- sample(Y, replace=F)# Shuffle Y

# Regression analysis
# Optimize alpha below
grid=exp(seq(5,-6,length=250))
fold_id=sample(rep(seq(10),length=length(Y_sh)))
table(fold_id)

alphaslist<-seq(0.2, 1, length=10)
alphaslist
registerDoParallel(4)
elasticnet<-lapply(alphaslist, function(a){cv.glmnet(X, Y_sh, alpha=a, foldid=fold_id, lambda=grid, parallel = TRUE)})
mypar(3,4)
...
lambdas <- replicate (50, {
  elasticnet<-cv.glmnet(X, Y_sh, alpha = a, nfolds = 10, lambda=grid, parallel = TRUE)
...
(mol <- mean(lambdas))
# ->lambda=77.47??
#Will keep alpha and lambda the same as All_vs_HSP90 lambda=0.075
mol<-0.075

# boostrap 200 times
grid=exp(seq(2,-6,length=182))
boot_betas <- matrix(data=NA, nrow=ncol(X)+1, ncol=200)
X_resample <- matrix(data=NA, nrow=length(Y_sh), ncol=ncol(X))
Y_resample <- matrix(data=NA, nrow=length(Y_sh), ncol=1)
for(i in 1:200){idx <- sample(length(Y_sh), replace = T)
for(j in 1:length(Y_sh)){X_resample[j,] <- X[idx[j],]; Y_resample[j] <- Y_sh[idx[j]]}
...
write.csv(result, file="All_vs_HSP90_Shuffle_result.csv")

# boostrap 200 times again
boot_betas2 <- matrix(data=NA, nrow=ncol(X)+1, ncol=200)
X_resample2 <- matrix(data=NA, nrow=length(Y_sh), ncol=ncol(X))
Y_resample2 <- matrix(data=NA, nrow=length(Y_sh), ncol=1)
for(i in 1:200){idx <- sample(length(Y_sh), replace = T)
for(j in 1:length(Y_sh)){X_resample2[j,] <- X[idx[j],]; Y_resample2[j] <- Y_sh[idx[j]]}
...
write.csv(result2, file="All_vs_HSP90_Shuffle_result2.csv")

# boostrap 200 times yet again
boot_betas3 <- matrix(data=NA, nrow=ncol(X)+1, ncol=200)
X_resample3 <- matrix(data=NA, nrow=length(Y_sh), ncol=ncol(X))
Y_resample3 <- matrix(data=NA, nrow=length(Y_sh), ncol=1)
for(i in 1:200){idx <- sample(length(Y_sh), replace = T)
for(j in 1:length(Y_sh)){X_resample3[j,] <- X[idx[j],]; Y_resample3[j] <- Y_sh[idx[j]]}
...
write.csv(result3, file="All_vs_HSP90_Shuffle_result3.csv")

# heatmap
result_at<-read.csv("All_vs_HSP90_Shuffle_result_at.csv")
result_at
dim(result_at)
...
CV_B <- matrix(data=NA, nrow=length(Y_sh), ncol=10)
for(j in 1:10){
  idx <- createFolds(Y_sh, k=10)
  for(i in 1:10){
    fit <- glmnet(X[-idx[[i]], ], Y_sh[-idx[[i]]], alpha = a, lambda=grid)
