...
fold_id=sample(rep(seq(8),length=length(Y_sh)))
table(fold_id)

alphaslist<-seq(0.05, 0.95, length=10)
alphaslist
registerDoParallel(4)
elasticnet<-lapply(alphaslist, function(a){cv.glmnet(X, Y_sh, alpha=a, nfold=8, foldid=fold_id, lambda=grid, parallel = TRUE)})
mypar(3,4)
...
lambdas <- replicate (50, {
  elasticnet<-cv.glmnet(X, Y_sh, alpha = a, nfolds = 8, lambda=grid, parallel = TRUE)
...
write.csv(result, file="glioma_vs_HSP90_Shuffle_result.csv")
...
write.csv(result2, file="glioma_vs_HSP90_Shuffle_result2.csv")
...
write.csv(result3, file="glioma_vs_HSP90_Shuffle_result3.csv")
