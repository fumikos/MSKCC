...
glioma_HSP90[1:5,]
glioma_HSP90[order(glioma_HSP90[,3]),]

#Also, extract Activity Area data for HSP90 inhibitor in all cells
...
rearranged = glioma_Exp[,idx]#extract expression data of cells that have HSP90i data and reorder to match HSP90 data order
dim(rearranged)
colnames(rearranged)
...
# Look at NQO1 expression
mypar(1,1)
hist(rearranged["NQO1",], breaks = 50)
plot(rearranged["NQO1",order(glioma_HSP90[,3])])
abline(10,0)
NQ_high<-rearranged[,rearranged["NQO1",]>10]
dim(NQ_high)# 25 cells have NQO1 expression higher than 10
row.names(NQ_high)<-row.names(CCLE_Exp_minus1)# change back row names to probe ID
glioma_HSP90_NQ_high<-glioma_HSP90[rearranged["NQO1",]>10,]
dim(glioma_HSP90_NQ_high)
mypar(1,2)
plot(sort(glioma_HSP90[,3]))
plot(sort(glioma_HSP90_NQ_high[,3]))
mypar(1,2)
plot(sort(HSP90[,3]))
plot(sort(glioma_HSP90[,3]))# glioma cells are mostly "intermediate"


#Assign resistant and sensitive cells
dim(glioma_HSP90_NQ_high)
HSP90_P1<-glioma_HSP90_NQ_high[order(glioma_HSP90_NQ_high[,3])[seq(1,25,2)],]# split data into 2 partitions
HSP90_P2<-glioma_HSP90_NQ_high[order(glioma_HSP90_NQ_high[,3])[seq(2,24,2)],]
dim(HSP90_P1)
dim(HSP90_P2)
res<-HSP90_P1[order(HSP90_P1[,3])[1:5],]
sen<-HSP90_P1[order(HSP90_P1[,3])[9:13],]
mypar(1,2)
plot(sort(glioma_HSP90_NQ_high[,3]))
plot(c(res[,3],sen[,3]))
...
colnames(res_exp)# 5 resistant cells
colnames(sen_exp)# 5 sensitive cells
...
head(sen_sig)
write.table(sen_sig$SYMBOL, file="glioma_sen_limma_NQ.txt", quote=F, row.names=F, col.names=F)

res_sig<-gene_name[fit$coefficients[,2]<0,]
dim(res_sig)
res_sig<-res_sig[order(fit$p.value[fit$coefficients[,2]<0, 2])[1:100],]
head(res_sig)
write.table(res_sig$SYMBOL, file="glioma_res_limma_NQ.txt", quote=F, row.names=F, col.names=F)
...
dim(X)
sig<-order(fit$p.value[,2])[1:49]
#sig<-c(row.names(sen_sig)[1:10],row.names(res_sig)[1:10])
class(sig)
sig<-as.numeric(sig)
X3<-X[sig,]# just use top 50
dim(X3)# 50 genes
X3[1:10,1:5]
X3<-t(X3)
...
dim(rearranged)
head(rearranged)
row.names(rearranged)<-row.names(NQ_high)
X4 <- rearranged[sig,]
dim(X4)
X4<-t(X4)
dim(X4)
...
# identify min of p-value
pvals<-vector("numeric",200)
for(i in 10:200){
  sig<-order(fit$p.value[,2])[1:i]
  X6 <-X[sig,]
  X6<-t(X6)
  model3 <- naiveBayes(X6, Y)
  X7 <- rearranged[sig, -c(res_idx2,sen_idx2)]
  X7<-t(X7)
  pred4 <- predict(model3, X7, type = "raw")
  sen_pred3<-pred4[,2]
  res_pred3<-pred4[,1]
  pvals[i]<-t.test(HSP90_2[sen_pred3==1,3],HSP90_2[res_pred3==1,3])$p.value
}
mypar(1,1)
plot(pvals)
pvals
...
# Use Cancer Genome Project data to validate

CGP_S1<-read.csv("G:/My Documents/CCLE/CGP_data/S1.csv")
class(CGP_S1)
dim(CGP_S1)
CGP_S1[1:5,1:10]
table(CGP_S1$Cancer.type)
CGP_glioma=CGP_S1[CGP_S1$Cancer.type=="glioma",]
dim(CGP_glioma)
CGP_glioma[,1:10]
CGP_glioma_HSP90<-CGP_glioma[,c(1,which(grepl("AAG", colnames(CGP_glioma)))[1],which(grepl("AUY922", colnames(CGP_glioma)))[1])]
class(CGP_glioma_HSP90[,2])
CGP_glioma_HSP90[,2]<-as.numeric(as.character(CGP_glioma_HSP90[,2]))
CGP_glioma_HSP90[,3]<-as.numeric(as.character(CGP_glioma_HSP90[,3]))
plot(CGP_glioma_HSP90[,2], CGP_glioma_HSP90[,3])
complete.cases(CGP_glioma_HSP90)
cor(CGP_glioma_HSP90[complete.cases(CGP_glioma_HSP90),2], 
    CGP_glioma_HSP90[complete.cases(CGP_glioma_HSP90),3])

IC50<-CCLE_Drug_all[glioma_HSP90_idx,11]
glioma_HSP90<-cbind(glioma_HSP90, IC50)
